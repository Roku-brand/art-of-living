...
// ========== UI シェル（要件：ヘッダーは≪トップ≫≪マイページ≫、タブは2つ） ==========
...
function renderList(osKey){
  ...
  let cards = sortById(DATA.byOS[currentOS] ?? []);
  const tags = buildTagSet(cards);

  // 要件：処世術一覧タブはデフォルト人生OS、ヘッダーの下にOS切替を配置
  view.innerHTML = `
    <div class="card" style="padding:14px;">
      <div class="row">
        <div>
          <div style="font-size:18px; margin-bottom:4px;">処世術一覧</div>
          <div class="small">デフォルト：人生OS。OS切替はヘッダー直下。</div>
        </div>
        <span class="badge">${escapeHtml(meta?.title ?? "人生OS")}</span>
      </div>
    </div>
...
function renderDetail(id){
  renderShell("list"); // 詳細は「処世術一覧」系として扱う
  const view = $("#view");
...
// ========== マイページ（β） ==========
function renderMy(){
  renderShell("top"); // 下タブは2つなので、Myはヘッダー導線として扱う
  const view = $("#view");
